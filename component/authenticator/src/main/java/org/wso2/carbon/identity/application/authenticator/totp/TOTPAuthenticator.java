...
public class TOTPAuthenticator extends AbstractApplicationAuthenticator implements LocalApplicationAuthenticator {
	...
	/**
     * This method is overridden to check additional condition like whether request is having sendToken,
     * token parameters, generateTOTPToken and authentication name.
     *
    ...
    public AuthenticatorFlowStatus process(HttpServletRequest request, HttpServletResponse response,
        ...
        } else if (StringUtils.isNotEmpty(request.getParameter(TOTPAuthenticatorConstants.ENABLE_TOTP))) {
    ...
    protected void initiateAuthenticationRequest(HttpServletRequest request, HttpServletResponse response,
        ...
        String loginPage =  IdentityUtil.getServerURL(TOTPAuthenticatorConstants.LOGIN_PAGE, false, false);
        String errorPage =  IdentityUtil.getServerURL(TOTPAuthenticatorConstants.ERROR_PAGE, false, false);
        String retryParam = "";
        try {
            ...
            String totpLoginPageUrl = loginPage + ("?sessionDataKey=" + context.getContextIdentifier()) +
                                    "&authenticators=" + getName() + "&type=totp" + retryParam + "&username=" +
                                    username;
            String totpErrorPageUrl = errorPage + ("?sessionDataKey=" + context.getContextIdentifier()) +
            ...
            throw new AuthenticationFailedException("Error when redirecting the TOTP login response, user : "
                                                    + username, e);
        ...
        try {
            ...
            if (userRealm != null) {
                Map<String, String> UserClaimValues = userRealm.getUserStoreManager().getUserClaimValues(
                        tenantAwareUsername,new String[]{TOTPAuthenticatorConstants.SECRET_KEY_CLAIM_URL},null);
    ...
    private boolean isValidTokenLocalUser(int token, String username, AuthenticationContext context)
        ...
        try {
            ...
            TOTPAuthenticatorConfig.TOTPAuthenticatorConfigBuilder totpAuthenticatorConfigBuilder = new
                    TOTPAuthenticatorConfig.TOTPAuthenticatorConfigBuilder().setKeyRepresentation(encoding)
                                                                            .setWindowSize(windowSize)
                                                                            .setTimeStepSizeInMillis(timeStep);
            TOTPAuthenticatorCredentials
                    totpAuthenticator = new TOTPAuthenticatorCredentials(totpAuthenticatorConfigBuilder.build());
            tenantAwareUsername = MultitenantUtils.getTenantAwareUsername(username);
            UserRealm userRealm = TOTPUtil.getUserRealm(username);
            if (userRealm != null) {
                Map<String, String> userClaimValues = userRealm.getUserStoreManager().getUserClaimValues
                        (tenantAwareUsername,new String[]{TOTPAuthenticatorConstants.SECRET_KEY_CLAIM_URL}, null);
                String secretKey =TOTPUtil.decrypt(userClaimValues.get(TOTPAuthenticatorConstants.SECRET_KEY_CLAIM_URL));
