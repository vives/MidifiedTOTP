...
public class TOTPKeyGenerator {
	 ...
	 */
	public static String createUrlWithQRCode(String username, boolean refresh, AuthenticationContext context)
		...
		try {
			...
			if (userRealm != null) {
				...
				String qrCodeURL =
						"otpauth://totp/" + tenantDomain + ":" + tenantAwareUsername + "?secret=" + secretKey +
						"&issuer=" + tenantDomain;
	...
	/**
	 * Remove the stored secret key ,qr code url and encoding method from user claims.
	 *
	 * @param username username of the user
	...
	public static boolean resetLocal(String username) throws TOTPException, AuthenticationFailedException {
		try {
			...
			if (userRealm != null) {
				...
				throw new TOTPException("Can not find the user realm for the given tenant domain : " +
	...
	private static TOTPAuthenticatorKey generateKey(String tenantDomain, AuthenticationContext context)
		...
		TOTPAuthenticatorConfig.TOTPAuthenticatorConfigBuilder configBuilder =
				new TOTPAuthenticatorConfig.TOTPAuthenticatorConfigBuilder().setKeyRepresentation(encoding);
		TOTPAuthenticatorCredentials totpAuthenticator = new TOTPAuthenticatorCredentials(configBuilder.build());
